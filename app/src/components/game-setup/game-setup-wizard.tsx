...
import {
	...
	Mode,
	Difficulty,
} from "@/lib/validations/game-setup";
...
import { CreateGameTransactionData } from "@/types/transactions";
...
export function GameSetupWizard() {
	...
	const getStepTitle = (step: number) => {
		switch (step) {
			case 1:
				return t("game.setup.mode.title");
			case 2:
				return t("game.setup.players.title");
			case 3:
				return t("game.setup.color.title");
			case 4:
				return t("game.setup.difficulty.title");
			case 5:
				return t("game.setup.timer.title");
			case 6:
				return t("game.setup.password.title");
			case 7:
				return t("game.setup.subtitle");
			default:
				return "";
		}
	};
	...
	const handleSubmit = async (data: GameSetupFormData) => {
		...
		try {
			...
			if (!currentAccount?.address) {
				setIsLoading(false);
				showError(t("errors.walletNotConnected"));
				return;
			}

			// 1. Create game in database FIRST (without object_id)
			...
			const gameData = {
				...
				// No objectId - will be updated after blockchain transaction
			};
			...
			try {
				...
				const transactionId = `create_game-${game.id}-${Date.now()}-${Math.random().toString(36).substring(7)}`;

				// 3. Emit transaction event to Socket.IO server
				const transactionData: CreateGameTransactionData = {
					transactionId,
				...
				// 4. Subscribe to transaction result (one-time)
				const subscription = transactionResult$
					.pipe(
						filter((result) => result.transactionId === transactionId),
						take(1) // Only take the first matching result
					)
					.subscribe(async (result) => {
						if (result.status === "success" && result.objectId) {
							// Update game with object_id
							try {
								const { updateGameObjectId } = await import(
									"@/app/actions/game"
								);
								await updateGameObjectId({
									gameId: game.id as string,
									objectId: result.objectId,
								});
								showSuccess(t("toast.gameCreatedOnBlockchain"));
							} catch (error) {
								console.error("Failed to update game objectId:", error);
								// Don't show error - game was created, just objectId update failed
							}
						} else if (result.status === "error") {
							showError(result.error || t("errors.failedCreateGameBlockchain"));
						}
					});

				// Cleanup subscription on component unmount or when transaction completes
				// The subscription will auto-unsubscribe after take(1), but we keep a ref for manual cleanup if needed

				// 5. Show loading notification
				showSuccess(t("toast.gameCreated"));

				// Enqueue mints AFTER game is successfully created on blockchain
				if (currentPlayer) {
					console.log("enqueueMint", currentPlayer);
					await enqueueMint("first_game", currentPlayer);
					await enqueueMint("first_game_created", currentPlayer);
				}

				// 7. Reset loading state before navigation
				setIsLoading(false);

				// 8. Navigate immediately (optimistic navigation)
				...
				setIsLoading(false);
				const errorMessage =
					error instanceof Error ? error.message : t("errors.failedCreateGame");
	...
	return (
		<div className="max-w-2xl mx-auto px-4">
			<Card className="w-full min-w-[20rem] sm:min-w-[24rem] max-w-[28rem] mx-auto">
				<CardHeader>
					<CardTitle className="text-center">{t("game.setup.title")}</CardTitle>
				...
				<CardContent className="space-y-4 sm:space-y-6">
					...
					<div className="flex flex-col sm:flex-row justify-between gap-2 sm:gap-0">
						...
						{currentStep === 7 ? (
							...
							>
								{isLoading
									? t("game.setup.loading", { ns: "common" })
									: t("game.setup.finish", { ns: "common" })}
							</Button>
						) : (
							<Button
								type="button"
